...
class NewSessionViewModel(app: Application) : AndroidViewModel(app) {
    ...
    val siiSubs = MutableLiveData(0)
    val siiExpSec = MutableLiveData(0)
    val lextSubs = MutableLiveData(0)
    val lextExpSec = MutableLiveData(0)
    val custom1Subs = MutableLiveData(0)
    val custom1ExpSec = MutableLiveData(0)
    val custom2Subs = MutableLiveData(0)
    val custom2ExpSec = MutableLiveData(0)

    val lproTime = MutableLiveData("00:00")
    val haTime = MutableLiveData("00:00")
    val oiiiTime = MutableLiveData("00:00")
    val siiTime = MutableLiveData("00:00")
    val lextTime = MutableLiveData("00:00")
    val custom1Time = MutableLiveData("00:00")
    val custom2Time = MutableLiveData("00:00")
    ...
    // Configuración de filtros desde Ajustes
    val showSii get() = prefs.getBoolean("show_sii", true)
    val showLext get() = prefs.getBoolean("show_lext", true)
    ...
    fun recalcTimes() {
        ...
        siiTime.value = formatTime(sSec)
        lextTime.value = formatTime(leSec)
        custom1Time.value = formatTime(c1Sec)
    ...
    fun saveSession() = viewModelScope.launch {
        ...
        if (obj == null && name.isNotBlank()) {
            repo.insertObject(com.astrolog.app.data.entity.AstroObject(name = name, status = "En curso"))
